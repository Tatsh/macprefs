...
Fixes mypy type errors and eliminates `Any` usage in the macprefs project.
...
1. Fix all mypy errors reported by `uv run mypy macprefs`
...
Place new TypedDict classes in `macprefs/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`macprefs/typing.py` when shared.
...
1. Run `uv run mypy macprefs` and capture all errors.
...
4. Run `uv run mypy macprefs` again to verify fixes.
