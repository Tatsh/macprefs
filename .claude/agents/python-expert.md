...
- **Logging**: bascom sets up logging: `setup_logging()`. Only use `setup_logging` in entry points
  ...
  def my_command(debug: bool = False) -> None:
      """Do something."""
      setup_logging(debug=debug, loggers={'macprefs': {}})
  ```

  Additional third-party loggers can be added as needed:

  ```python
  setup_logging(debug=debug, loggers={'macprefs': {}, 'urllib3': {}, 'soupsieve': {}})
  ```
...
- `` :py:mod:`macprefs.string` `` for modules
- `` :py:func:`macprefs.string.slugify` `` for functions
- `` :py:class:`macprefs.typing.ProbeDict` `` for classes
- `` :py:meth:`MyClass.my_method` `` for methods
- Use `~` to shorten the displayed name: `` :py:func:`~macprefs.string.slugify` `` renders as
  `slugify`
