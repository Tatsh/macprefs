    ...
    for value in pl_list:
        if not isinstance(value, bytes):
            if isinstance(value, dict):
                ret[index] = cast(MutablePlistRoot, await remove_data_fields(value))
            elif isinstance(value, list):
                ret[index] = cast(MutablePlistList, await remove_data_fields_list(value))
    ...
    for key, value in root.items():
        if not isinstance(value, bytes):
            if isinstance(value, list):
                ret[key] = cast(MutablePlistList, await remove_data_fields_list(value))
            elif isinstance(value, dict):
                ret[key] = cast(MutablePlistRoot, await remove_data_fields(value))
